...
class DailyCrawler:
   ...
   def crawl_index(self, begin_date=None, end_date=None):
        ...
        for code in index_codes:
            ...
            # 保存数据
            self._save(code, df_daily, self.daily, {'index': True})

   def _save(self, code, df_daily, collection, extra_fields=None):
        """
        ...
        for df_index in df_daily.index:
            doc = dict(df_daily.loc[df_index]) # 将 DF 中的 one转变为dict
            # 设置股票代码
            doc['code'] = code
   ...
   def crawl(self, begin_date=None, end_date=None):
        ...
        for code in codes:
            ...
            self._save(code, df_daily, self.daily, {'index': False})

            # 抓取后复权的价格
            df_daily_hfq = ts.get_k_data(code, autype='hfq', start=begin_date, end=end_date)
            self._save(code, df_daily_hfq, self.daily_hfq, {'index': False})
